...
describe ('First test suite', () => {
    ...
    it('radio buttons', () => {
        ...
        cy.contains('nb-card', 'Using the Grid').find('[type="radio"]').then( raddioButtons => {
            cy.wrap(raddioButtons).eq(0).check({force:true}).should('be.checked') 
            cy.wrap(raddioButtons).eq(1).check({force:true})
            cy.wrap(raddioButtons).eq(0).should('not.be.checked') 
            cy.wrap(raddioButtons).eq(1).should('be.checked')
        })
